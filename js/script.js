window.onload = function() {
            ...
            for (let k = tetris[i].length; k >= 0; k--) {
                ...
                if (tetris[i][k] < 10) {
                    if (tetris[i][k] != 0 && tetris[i][k + 1] == 0) {
                        if (tetris[i][k] != 0 && tetris[i + 1][k] == 0 && tetris[i][k + 1] == 0) {
    ...
    function tetrisLeft() {
        for (let i = tetris.length - 1; i >= 0; i--) {
            for (let k = 0; k < tetris[i].length; k++) {
                ...
                if (tetris[i][k] < 10) {
                    if (tetris[i][k] != 0 && tetris[i][k - 1] == 0) {
                        if (tetris[i][k] != 0 && tetris[i][k - 1] == 0 && tetris[i + 1][k] == 0) {
                            tetris[i][k - 1] = tetris[i][k];
                            tetris[i][k] = 0;
                        }
                    }
    ...
    function checkLine() {
        for (let i = tetris.length - 1; i >= 0; i--) {
            for (let k = 0; k < tetris[i].length; k++) {
                if (tetris[i][k] > 1 && tetris[i][k + 1] != undefined && tetris[i][k + 2] != undefined) {
